...
### ⚠️ Gotcha 3 — protocollo sbagliato sulla riga SNAT del Client

```
iptables -t nat -A POSTROUTING -p tcp -i eth1 -s 192.168.0.0/24 -o eth3 -j SNAT --to-source 130.136.5.15
```
Questa riga vuole mascherare l'uscita DNS/NTP dei client verso Internet — ma DNS e NTP sono **UDP**
(le due coppie FORWARD subito sopra sono infatti correttamente `-p udp`), mentre qui il match è
`-p tcp`: non intercetta i pacchetti UDP a cui è destinata. Probabile anche questo un
copia-incolla da un contesto TCP (SMTP) non adattato al protocollo. Per intento, andrebbe `-p udp`
o nessun vincolo di protocollo.
