...
| Keyword | Significato |
|---|---|
| `content:"<stringa>"` | Match su una stringa/pattern **fissa e nota** nel payload |
| `pcre:"/<regex>/<flag>"` | Match con **espressione regolare** invece di una stringa fissa — per pattern variabili/con parte mutevole (vedi § 4.3.4 per quando preferirlo a `content`) |
| `http_uri` | Restringe il `content` precedente all'URI HTTP (buffer keyword, subito dopo il content che modifica) |
| `dns.query` | Buffer keyword: sposta il `content`/`pcre` successivo sul **nome di dominio interrogato** in una query DNS (vedi § 4.3.3) |
| `flags:S` | Match sui flag TCP grezzi dell'header (`S`=SYN, `A`=ACK, `R`=RST, `F`=FIN...) — utile per scan/flood dove non c'è payload da matchare (vedi § 4.3.2) |
...
| `threshold: type threshold, track by_src, count N, seconds T` | Scatena l'alert solo se una sorgente supera N match in T secondi — utile per volumi/scan invece di un content specifico (esempio completo in § 4.3.2) |
| `detection_filter: track by_src, count N, seconds T` | Come `threshold type threshold` ma **continua** ad allertare su ogni match successivo al superamento della soglia, invece di azzerare il contatore (vedi § 4.3.2) |
...
### 4.3.1 Quando un `content` fisso non basta

Le tre keyword sotto (`threshold`/`detection_filter`, `dns.query`, `pcre`) coprono i casi in cui
un semplice `content` fisso non basta: un attacco **volumetrico** (conta i pacchetti, non il loro
contenuto), un match sul **nome DNS interrogato**, un pattern **variabile** invece di una stringa
nota. Sotto, una regola completa e pronta per ciascuno.

### 4.3.2 Regola per un attacco volumetrico/rate-based (SYN flood, DDoS) — `threshold`

Quando l'attacco **non ha un payload da matchare** ma è definito dal *volume* (tanti pacchetti in
poco tempo), il `content` non serve: si conta la frequenza con `threshold`. Caso tipico: **SYN
flood** — pacchetti TCP con solo il flag SYN, mai completati in handshake, a raffica verso una
porta. Il flag SYN grezzo lo isoli con `flags:S` (§ 4.7 per la decodifica dei flag).

**Regola pronta (SYN flood verso la porta 80 di una vittima):**
```
alert tcp any any -> $HOME_NET 80 (msg:"Possibile SYN flood verso porta 80"; flags:S; threshold: type threshold, track by_dst, count 100, seconds 10; sid:1000010; rev:1;)
```
Legge: *alza un alert se arrivano più di 100 SYN in 10 secondi verso la stessa destinazione sulla
porta 80.*

**`track by_src` vs `track by_dst` — la scelta che conta:**
- **`track by_dst`**: conta i pacchetti **per IP di destinazione**, sommando tutte le sorgenti. È
  la scelta per un flood **distribuito** (DDoS: tante sorgenti diverse, spesso spoofate, che
  colpiscono una vittima — nessuna singola sorgente supererebbe la soglia da sola, ma il totale
  sulla vittima sì). Corrisponde al quarto/DDoS caso già visto nel modello.
- **`track by_src`**: conta **per IP sorgente**. È la scelta quando il volume viene da **un solo
  host** aggressivo (un port scan, un flood da singola sorgente): fai scattare l'alert su chi
  supera la soglia, senza flaggare il traffico legittimo degli altri.
- `count N` = quanti match; `seconds T` = finestra temporale su cui contarli. Tarali sul pcap: se
  il traffico legittimo verso quella porta è ~5 richieste/minuto e l'attacco ne fa centinaia in
  pochi secondi, un `count 100, seconds 10` separa nettamente i due senza falsi positivi.

**`threshold` vs `detection_filter` — quando preferire l'uno o l'altro:**
- **`threshold: type threshold, ...`** azzera il contatore dopo aver allertato: ottieni **un
  alert per finestra** superata — pulito per il report (dici "c'è stato un flood", non ti servono
  mille righe identiche). È la scelta di default per un compito d'esame.
- **`detection_filter: track ..., count N, seconds T`** (senza `type`) invece **continua ad
  allertare su ogni pacchetto** una volta superata la soglia: utile se vuoi *tutti* i pacchetti
  dell'attacco loggati in `eve.json` (es. per contarli con precisione), non solo il primo.
- In pratica all'esame: `threshold type threshold` se vuoi la prova che l'attacco esiste;
  `detection_filter` se la consegna chiede di **registrare/intercettare ogni** pacchetto malevolo.

Verifica poi con i grep di § 4.6, esattamente come per le regole `content`.

### 4.3.3 Regola content-based su DNS (tunneling/exfiltration) — `dns.query`

§ 3.8 dà i criteri per **riconoscere** un DNS tunneling (sottodomini lunghi e "casuali", query TXT
frequenti, sottodominio diverso a ogni query verso lo stesso dominio base). Per **scriverne la
regola** serve la buffer keyword **`dns.query`**, che sposta il `content`/`pcre` successivo sul
**nome di dominio interrogato** (in chiaro: per Suricata `mail.google.com` è già testo, non wire
format). Il protocollo nell'header è `dns`, direzione client→server (`any any -> any any` va bene
sul pcap offline).

**Variante A — dominio base sospetto noto** (l'hai identificato in § 3.8):
```
alert dns any any -> any any (msg:"Possibile DNS tunneling/exfiltration"; dns.query; content:".attacker.xyz"; nocase; sid:1000011; rev:1;)
```
Matcha ogni query che contiene quel dominio, qualunque sia il sottodominio (i dati esfiltrati
cambiano il sottodominio a ogni richiesta ma il dominio base resta fisso). `nocase` = ignora
maiuscole/minuscole.

**Variante B — sottodominio anomalo, senza conoscere il dominio** (pattern, non stringa fissa —
usa `pcre`, § 4.3.4):
```
alert dns any any -> any any (msg:"Query DNS con sottodominio anomalo (possibile tunneling)"; dns.query; pcre:"/^[a-z0-9]{25,}\./i"; sid:1000012; rev:1;)
```
Legge: *un'etichetta iniziale di 25+ caratteri alfanumerici seguita da un punto* — la firma di un
sottodominio codificato base32/64, che un nome "umano" (`www.`, `mail.`) non ha mai. Alza la soglia
di lunghezza se generi falsi positivi su nomi legittimi lunghi.

**Se il segnale è la frequenza** (stesso dominio interrogato a raffica, tipico dell'exfiltration a
pezzi) puoi combinare con `threshold` (§ 4.3.2): `dns.query; content:".attacker.xyz"; threshold:
type threshold, track by_src, count 20, seconds 5;`.

### 4.3.4 `pcre` — quando il pattern non è una stringa fissa

`content` matcha una **stringa esatta e nota**. Quando il pattern d'attacco è **variabile** (una
parte cambia: cifre, lunghezza, caratteri casuali) usi `pcre` con una regex:
```
pcre:"/<regex>/<flag>"
```
Esempio (payload con `A` ripetute di lunghezza qualunque seguite dal marcatore `BBBB`, la variante
"padding di lunghezza variabile" del buffer-overflow probe di § 3.3):
```
alert tcp any any -> $HOME_NET any (msg:"BOF probe padding variabile"; pcre:"/A{20,}BBBB/"; sid:1000013; rev:1;)
```
Flag utili dopo la seconda `/`: **`i`** (case-insensitive), **`R`** (match *relativo*, subito dopo
il `content` precedente), **`s`** (`.` matcha anche newline). Su un buffer sticky (es. dopo
`dns.query`, § 4.3.3) il `pcre` si applica a quel buffer, non all'intero payload.

**Criterio `content` vs `pcre`:**
- **`content`** — la stringa è **fissa e la conosci** (`FLAG`, `ABCD`, `sec:sec`, un dominio noto).
  Preferiscilo sempre quando basta: è più veloce e più leggibile. È il caso dei 5 modelli risolti.
- **`pcre`** — la stringa **ha una parte che cambia** e devi descriverla, non elencarla: lunghezza
  variabile (`A{20,}`), cifre in mezzo, sottodomini casuali, alternative (`(cmd|exec|system)`).
  Costa di più in performance ma è l'unico modo di matchare un pattern che non è una costante.
- Regola pratica: parti sempre da `content`; passa a `pcre` **solo** se il pattern ha un pezzo
  intrinsecamente mutevole che `content` non può catturare.
